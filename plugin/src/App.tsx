...
export function App() {
  ...
  const statusTone =
    receiverStatus?.status === "success" ? "#1f7a1f"
    : receiverStatus?.status === "failed" ? "#a12c2c"
    : receiverStatus?.status === "running" || receiverStatus?.status === "queued" ? "#8a6500"
    : "#666666"
  ...
  const filteredPlacements = useMemo(() => {
    ...
    return placements.filter((item) => {
      const matchesReadiness = readinessFilter === "all" || (item.readiness || "unknown") === readinessFilter
      const matchesCategory = categoryFilter === "all" || (item.category || "uncategorized") === categoryFilter
      const haystack = [item.id, item.name, item.category, item.sourceFile, item.importHint, item.readinessLabel]
        .filter(Boolean).join(" ").toLowerCase()
      return matchesReadiness && matchesCategory && (!query || haystack.includes(query))
    })
  }, [placements, readinessFilter, categoryFilter, searchQuery])

  const groupedPlacements = useMemo(() => {
    const order = ["ready", "generated-not-imported", "missing-module-url", "missing-generated-file"]
    return order
      .map((groupKey) => ({
        key: groupKey,
  ...
  return (
    <main style={{ fontFamily: "Inter, sans-serif", padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ padding: 10, borderRadius: 10, background: "rgba(0,0,0,0.04)", border: `1px solid ${statusTone}33` }}>
      ...
      <label style={{ display: "flex", alignItems: "center", gap: 10, padding: 10, borderRadius: 10, border: "1px solid rgba(0,0,0,0.08)" }}>
        <input type="checkbox" checked={useCheapModel} onChange={(e) => setUseCheapModel(e.target.checked)} />
        <span style={{ fontSize: 13 }}>Prefer cheap model when possible</span>
      </label>

      <div style={{ fontSize: 12, lineHeight: 1.45, opacity: 0.75, padding: 10, borderRadius: 10, background: "rgba(0,0,0,0.04)" }}>
        {providerHint}
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button
          onClick={handleSend}
          style={{ flex: 1, padding: "12px 14px", borderRadius: 12, border: "none", background: "#111111", color: "#ffffff", fontWeight: 600, cursor: "pointer" }}
        >
          Send to local pipeline
        </button>
        <button
          onClick={handleCopy}
          style={{ padding: "12px 14px", borderRadius: 12, border: "1px solid rgba(0,0,0,0.12)", background: "#ffffff", color: "#111111", fontWeight: 600, cursor: "pointer" }}
        >
          Copy
        </button>
      </div>
      ...
      {receiverArtifacts?.artifacts && (
        <div style={{ marginTop: 4, padding: 12, borderRadius: 12, background: "rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.08)", display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {receiverArtifacts.artifacts.reports.length ? (
                receiverArtifacts.artifacts.reports.map((item) => (
                  <button key={item} onClick={() => handleRead(item)}
                    style={{ textAlign: "left", padding: "8px 10px", borderRadius: 10, border: "1px solid rgba(0,0,0,0.08)", background: "#fff", cursor: "pointer", fontSize: 12 }}>
                    {item}
                  </button>
                ))
              ) : (
            ...
            <div style={{ fontSize: 12, opacity: 0.78, maxHeight: 100, overflow: "auto" }}>
              {receiverArtifacts.artifacts.motion.length
                ? receiverArtifacts.artifacts.motion.map((item) => (
                    <div key={item.id}>{item.id} — {item.implementation}</div>
                  ))
        ...
        <div style={{ marginTop: 4, padding: 12, borderRadius: 12, background: "rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.08)", display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {groupedPlacements.length ? (
            groupedPlacements.map((group) => (
              <div key={group.key} style={{ display: "flex", flexDirection: "column", gap: 8, paddingTop: 4 }}>
                ...
                {group.items.map((item) => {
                  ...
                  const draftUrl = urlDraft[item.id] ?? item.moduleUrl ?? ""
                  const urlChanged = draftUrl.trim() !== (item.moduleUrl ?? "").trim()

                  return (
                    <div key={item.id} style={{ padding: 10, borderRadius: 10, border: "1px solid rgba(0,0,0,0.08)", background: "#fff", display: "flex", flexDirection: "column", gap: 8 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}>
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 600 }}>{item.name}</div>
                          <div style={{ fontSize: 11, opacity: 0.65 }}>{item.id}</div>
                        </div>
                        <div style={{ fontSize: 11, fontWeight: 700, color: tone.text, background: tone.bg, padding: "6px 8px", borderRadius: 999 }}>
                          {item.readinessLabel || "Unknown"}
                        </div>
                      </div>

                      <div style={{ fontSize: 11, opacity: 0.85 }}>Category: {item.category || "Unknown"}</div>
                      <div style={{ fontSize: 11, opacity: 0.75 }}>Source: {item.sourceFile || item.generatedFile || "not found"}</div>
                      <div style={{ fontSize: 11, opacity: 0.75 }}>Hint: {item.importHint || "No hint"}</div>
                      <div style={{ fontSize: 11, opacity: 0.75 }}>
                        {item.width || 0} × {item.height || 0} · {formatBytes(item.fileSize)} · {formatDate(item.updatedAt)}
                      </div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                          ...
                          <div style={{ display: "flex", gap: 6 }}>
                            <input
                              ...
                              style={{ flex: 1, padding: "6px 8px", borderRadius: 8, border: "1px solid rgba(0,0,0,0.12)", fontSize: 11, minWidth: 0 }}
                            />
                            <button
                              onClick={() => handleSaveUrl(item)}
                              disabled={!urlChanged}
                              style={{ padding: "6px 10px", borderRadius: 8, border: "none", background: urlChanged ? "#111" : "#d8d8d8", color: urlChanged ? "#fff" : "#666", cursor: urlChanged ? "pointer" : "not-allowed", fontSize: 11, fontWeight: 600, whiteSpace: "nowrap" }}
                            >
                              Save
                            </button>
                          </div>
                        ...
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                          <button
                            ...
                            style={{ padding: "8px 10px", borderRadius: 10, border: "none", background: canInsert ? "#111" : "#d8d8d8", color: canInsert ? "#fff" : "#666", cursor: canInsert ? "pointer" : "not-allowed", fontSize: 12, fontWeight: 600 }}
                          >
                            Insert
                          </button>
                          {(item.sourceFile || item.generatedFile) && (
                            <button
                              ...
                              style={{ padding: "8px 10px", borderRadius: 10, border: "1px solid rgba(0,0,0,0.12)", background: "#fff", color: "#111", cursor: "pointer", fontSize: 12, fontWeight: 600 }}
                            >
                              Preview source
                            </button>
                          )}
            ...
            <div style={{ padding: 12, borderRadius: 10, background: "#fff", border: "1px solid rgba(0,0,0,0.08)", fontSize: 12, opacity: 0.75 }}>
      ...
      {previewContent && (
        <div style={{ marginTop: 4, padding: 12, borderRadius: 12, background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ fontSize: 13, fontWeight: 700 }}>{previewTitle}</div>
          <pre style={{ margin: 0, whiteSpace: "pre-wrap", wordBreak: "break-word", fontSize: 12, lineHeight: 1.45, maxHeight: 240, overflow: "auto", background: "rgba(0,0,0,0.03)", padding: 10, borderRadius: 10 }}>
            {previewContent}
          </pre>
        </div>
      )}
